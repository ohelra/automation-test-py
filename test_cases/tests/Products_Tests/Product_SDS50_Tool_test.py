...
from base_pages.pages.Products_Pages.Product_SDS50_Tool_Pages import ProductSDS50ToolPage
from utilities.custom_logger import LogMaker
from utilities.json_reader import JsonReader

@pytest.mark.ProductSDS50
class TestProductSDS50Tool:
    ...
    def class_setup(self, request, setup):
        ...
        request.cls.page = ProductSDS50ToolPage(driver)
    ...
    def test_verify_navigate_to_sds50_page(self):
        ...
        Step by step:
            ...
            2. Click SDS50 menu item
            3. Verify page title is 'INNOVA SDS50'
        Expected: Page title matches
        """
        self.logger.info("===== Verify Navigate to SDS50 Product Page =====")
        try:
            self.page.click_sds50_tool()
            title = self.page.get_title_sds50_text()
            assert title == "INNOVA SDS50", f"Expected 'INNOVA SDS50', got '{title}'"
            self.logger.passed("Navigated to SDS50 page successfully — title matches")
        except Exception as e:
            self.logger.failed(f"Failed to navigate to SDS50 page: {e}")
    ...
    def test_verify_thumbnails_show_different_images(self):
        ...
        try:
            self.page.click_sds50_tool()
            count = self.page.get_thumbnail_count_sds50()
            active_images = []
            for i in range(count):
                self.page.click_thumbnail_by_index_sds50(i)
                img_src = self.page.get_image_src_sds50()
    ...
    def test_verify_zoom_multi_clicks(self):
        ...
        try:
            self.page.click_sds50_tool()
            self.page.scroll_to_zoom_in_and_out_sds50()
            self.logger.info("Scroll to zoom in and out")
            initial_status = self.page.get_zoom_out_disabled_status_sds50()
            self.logger.info(f"Initial Zoom Out 'aria-disabled': {initial_status}")
            assert initial_status == "true"

            for i in range(3):
                self.page.click_zoom_in_sds50_button()
                status_after_zoom_in = self.page.get_zoom_out_disabled_status_sds50()
            ...
            for i in range(3):
                self.page.click_zoom_out_sds50_button()
                status_after_zoom_out = self.page.get_zoom_out_disabled_status_sds50()
    ...
    def test_verify_carousel_navigation_arrows(self):
        ...
        try:
            self.page.click_sds50_tool()
            initial_img_src = self.page.get_image_src_sds50()
            self.logger.info(f"Initial Image SRC: {initial_img_src}")
            self.page.click_carousel_next_sds50()
            self.logger.info(f"Click next button")
            next_img_src = self.page.get_image_src_sds50()
            ...
            self.page.click_carousel_prev_sds50()
            self.logger.info(f"Click previous button")
            prev_img_src = self.page.get_image_src_sds50()
    ...
    @pytest.mark.parametrize("sds50_tool", [JsonReader("product_sds50_tool.json").get_data("product_sds50")])
    def test_verify_name_features_are_correctly(self,sds50_tool):
        """
        Step by step:
             1. Navigate to SDS50 tool page
        ...
        try:
            self.page.click_sds50_tool()
            self.page.click_read_more_sds50_button()
            self.logger.info("Scrolled to Read more and clicked button")
            assert self.page.get_all_features_sds50_text() == sds50_tool["features"]
            time.sleep(1.5)
            self.page.click_read_more_sds50_button()
    ...
    def test_verify_click_secure_gateway_fca_navigate_to_article(self):
        ...
        Step by Step:
            ...
            5. Verify that navigate to Innova SDS50 - How to Unlock the Secure Gateway (SGW)
        """
        self.logger.info("===== Verify that Secure Gateway Unlock (FCA) to Innova SDS50 - How to Unlock the Secure Gateway (SGW) =====")
        try:
            self.page.click_sds50_tool()
            self.page.click_read_more_sds50_button()
            self.logger.info("Scrolled to Read more and clicked button")
            ...
            assert self.page.get_title_text_secure_gateway_fca() == "Innova SDS50 - How to Unlock the Secure Gateway (SGW)"
            self.logger.passed("Navigate to Innova SDS50 Unlock Secure Gateway (SGW) page")
        except Exception as e:
    ...
    def test_verify_complete_specs_and_feature_list_navigate_to_compare_diagnostic_product(self):
        ...
        try:
            ...
            self.page.click_read_more_sds50_button()
            self.logger.info("Scrolled to Read more and clicked button")
            self.page.click_complete_specs_feature_list_sds50()
    ...
    def test_verify_default_video_play_and_pause(self):
        ...
        try:
            self.page.click_sds50_tool()
            self.page.scroll_to_video_sds50()
            self.logger.info("Scroll to video")
            self.page.play_main_youtube_video_sds50()
            time.sleep(5)
            current_time = self.page.get_video_current_time_sds50()
            ...
            self.page.pause_youtube_video_sds50()
            self.logger.info("Pausing the video...")
            assert self.page.is_video_paused_sds50() is True
            ...
            self.page.switch_to_default_content_sds50()
    ...
    def test_verify_navigate_to_apple_store(self):
        ...
        try:
            self.page.click_sds50_tool()
            self.logger.info("Navigate to SDS50 page")
            self.page.scroll_to_store_sds50()
            self.logger.info("Scroll to store download app")
            self.page.navigate_new_tab_with_url(self.page.click_apple_link_sds50, "apps.apple.com")
    ...
    def test_verify_navigate_to_google_play_store(self):
        ...
        try:
            ...
            self.page.scroll_to_store_sds50()
            self.logger.info("Scroll to store download app")
            self.page.navigate_new_tab_with_url(self.page.click_google_link_sds50, "play.google.com")
